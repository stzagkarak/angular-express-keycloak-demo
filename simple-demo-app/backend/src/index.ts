...
// Dot not confuse the token names with OpenID Connect tokens
...
const generateTokens = (user: User) => {
  ...
  const SessionRefreshToken = jwt.sign(
  ...
  return { SessionAccessToken, SessionRefreshToken };
};
...
app.get(
  ...
  (req, res) => {
    ...
    const { SessionAccessToken, SessionRefreshToken } = generateTokens(user);

    // Set session refresh token as httpOnly cookie
    res.cookie("SessionRefreshToken", SessionRefreshToken, {
      httpOnly: true,
    ...
    res.redirect(`${frontendUrl}/auth/callback?token=${SessionAccessToken}`);
  }
);
...
// logout should also be forced when the initial refreshToken has expired. Not included in this demo.
...
// normally, the backend should try perform this operation automatically when the access token is expired
...
app.post("/api/auth/jwt/refresh", (req, res) => {
  const SessionRefreshToken = req.cookies.SessionRefreshToken;
  ...
  jwt.verify(
    ...
    (err: any, decoded: any) => {
      ...
      res.cookie("SessionRefreshToken", newSessionRefreshToken, {
        httpOnly: true,
        secure: process.env.NODE_ENV === "production",
        sameSite: "lax",
      ...
      return res.json({ SessionAccessToken });
    }
  );
